...
docker run -d --init \
    ...
    dclong/jupyterhub-golang /scripts/sys/init.sh
```
Use the image with the `next` tag (which is the testing/next version of dclong/jupyterhub-golang).
```
docker run -d --init \
    --platform linux/amd64 \
    --hostname jupyterhub-golang \
    ...
    -v "$(pwd)":/workdir \
    -v "$(dirname $HOME)":/home_host \
    dclong/jupyterhub-golang:next /scripts/sys/init.sh
```
